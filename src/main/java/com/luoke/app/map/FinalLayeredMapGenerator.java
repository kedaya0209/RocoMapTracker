...
public class FinalLayeredMapGenerator {
    ...
    private static final int SMOOTH_RADIUS = 15;   // 保持适中的平滑，避免过渡带过长导致稀疏
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("4. 像素合成：强化边缘厚度...");
            int[] finalPixels = new int[FULL_SIZE * FULL_SIZE];
            ...
            IntStream.range(0, FULL_SIZE * FULL_SIZE).parallel().forEach(i -> {
                ...
                if (maskVal <= 0.001f) {
                    ...
                    // --- 关键改进点 ---
                    // 使用开根号 Math.sqrt，让 maskVal 在离开 0 以后迅速跳升到高位
                    // 这样边缘就不会只有“薄薄一层”，而是立刻变得浓郁
                    float strongMask = (float) Math.sqrt(maskVal);

                    // 如果你觉得还是不够厚，可以再叠加一个保底权重
                    // strongMask = Math.min(1.0f, strongMask * 1.2f);
                    ...
                    finalPixels[i] = (255 << 24) | (rOut << 16) | (gOut << 8) | bOut;
                }
            });

            System.out.println("5. 导出大图...");
            saveImage(finalPixels, basePath + "Final_WorldMap_StrongEdge.png");
            System.out.println("成功！耗时: " + (System.currentTimeMillis() - startTime) / 1000.0 + "s");
